...
## Автоматические тесты: 60/60 ✅ (0 предупреждений сборки)
...
| **Правка состава → резерв заменяется (5), не накапливается (2+5=7)** | ✅ |
| **Не-USD источник (Prom) → отклонён даже при currency=USD** | ✅ |
| **Чексумма стабильна к формату чисел** (10 == 10.00) — нет ложного OrderModified | ✅ |
| **Транзиентная ошибка 1С (Locked) пробрасывается** → повтор в очереди, не «Помилка» | ✅ |
